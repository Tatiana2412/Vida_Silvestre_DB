...
CREATE OR REPLACE PROCEDURE registrar_visita(
    p_id_habitat INT,
    p_id_visitante INT,
    p_fecha_visita DATE
)
...
BEGIN
    ...
    SELECT 
            Descuento 
    INTO    v_descuento 
    FROM TIPO_VISITANTES TV
    JOIN VISITANTES V ON V.IDTipoVisitante = TV.ID
    WHERE V.ID = p_id_visitante;

    -- Llamar a la función para calcular el costo final
    v_costo_final := calcular_descuento(v_costo_base, v_descuento);

    -- Insertar la visita en la tabla Habitat_Visitantes con el costo calculado
    INSERT INTO HABITAT_VISITANTES (IDHabitat, IDVisitantes, FechaVisita, CostoFinal)
