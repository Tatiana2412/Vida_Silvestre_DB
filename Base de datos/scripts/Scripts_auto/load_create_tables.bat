...
set "PGPORT=5432"
set "PGUSER=adminzoo"
set "PGPASSWORD=lamus.1234*"
set "PGDATABASE=zoologico"

:: Ejecuta cada archivo SQL en orden

echo Ejecutando TableDefinitions.sql...
psql -h %PGHOST% -p %PGPORT% -U %PGUSER% -d %PGDATABASE% -f "C:\Users\Erwin Jaime\Desktop\GIT\GitHud\Doc -UP-ErwinJaime\BD-GB-ZOO\Scripts\ddl\create_tables.sql"
if %errorlevel% neq 0 (
